...
class NotificationService {
  ...
  /**
   * Envia notificação de atualização de nota para um aluno
   */
  ...
  /**
   * Envia notificação de atualização de nota em lote
   */
  async sendBatchUpdateNotification(data: BatchUpdateNotification): Promise<{ message: string; totalEnviados: number }> {
    const response = await fetch(`${API_URL}/notifications/batch-update`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to send batch update notification');
    }
